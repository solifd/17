...
cp -a /home/www/wordpress/wp-content/  $TMP
...
tar -zcvf - $TMP|openssl des3 -salt -k k3g5ZmSuIInJOR7MFcpy | dd of=wordpress$(date +"%Y%m%d").des3
...
sh /root/Dropbox-Uploader-master/dropbox_uploader.sh upload  /tmp/linodeatt/linodebackup/wordpress$(date +"%Y%m%d").des3

rm -rf /tmp/linodeatt/linodebackup/wordpress*
